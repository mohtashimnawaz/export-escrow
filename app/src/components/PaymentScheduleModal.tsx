...
import type { PaymentSchedule, PaymentMilestone, PaymentManager, MILESTONE_TEMPLATES } from '@/utils/paymentUtils';
import { PaymentManager as PaymentManagerClass } from '@/utils/paymentUtils';
...
export function PaymentScheduleModal({ 
  ...
  const [customMilestones, setCustomMilestones] = useState<Omit<PaymentMilestone, 'id' | 'status' | 'approvals'>[]>([]);
  ...
  const handleCreateSchedule = async () => {
    ...
    try {
      ...
      const milestonesWithAmounts = milestones.map(milestone => ({
        ...milestone,
        amount: (order.amount * milestone.percentage) / 100
      }));

      const schedule = paymentManager.createPaymentSchedule(
        order.id,
        order.amount,
        order.currency,
        milestonesWithAmounts
      );
